...
| Bileşen      | Binary / Süreç              | Port  | Systemd unit            |
| ------------ | --------------------------- | ----- | ----------------------- |
...
Örnek config: `deploy/nginx/qtss.conf.example`. Routing özet:

| Path             | Hedef                  | Açıklama                                  |
| ---------------- | ---------------------- | ----------------------------------------- |
| `/`              | web v1 (4173)          | Varsayılan, geriye dönük uyumlu           |
| `/v2/`           | web v2 (4174)          | Yeni Faz 8.0+ frontend                    |
| `/api/`          | qtss-api (8080)        | Ortak (v1 + v2 route'ları aynı binary'de) |
| `/health`, `/live`, `/ready`, `/metrics` | qtss-api (8080) | Probe / izleme                  |

```bash
sudo cp /app/qtss/deploy/nginx/qtss.conf.example /etc/nginx/conf.d/qtss.conf
sudo nano /etc/nginx/conf.d/qtss.conf   # server_name vb. düzenle
