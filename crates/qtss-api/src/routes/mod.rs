...
pub use v2_analiz::v2_analiz_router;
...
pub fn api_router(state: SharedState) -> Router<SharedState> {
    ...
    Router::new()
        ...
        .merge(v2_analiz_router().layer(from_fn(require_dashboard_roles)))
        ...
        // (exchange, segment, symbol, tf, kind) — no PII, no order
        // data — so a follow-up PR can re-introduce auth via a
        // short-lived signed connection token.
