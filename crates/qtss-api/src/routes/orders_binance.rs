...
pub fn orders_binance_read_router() -> Router<SharedState> {
    Router::new()
        .route("/orders/binance", get(list_my_orders))
        .route("/orders/binance/fills", get(list_my_fills))
}
...
#[derive(Deserialize)]
pub struct ListBinanceFillsQuery {
    #[serde(default = "list_binance_fills_default_limit")]
    limit: i64,
}

fn list_binance_fills_default_limit() -> i64 {
    200
}
...
async fn list_my_fills(
    Extension(claims): Extension<AccessClaims>,
    State(st): State<SharedState>,
    Query(q): Query<ListBinanceFillsQuery>,
