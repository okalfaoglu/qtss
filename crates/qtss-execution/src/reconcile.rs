...
fn parse_binance_open_order_ids(remote: &Value) -> Result<Vec<i64>, ExecutionError> {
    let arr = remote
        .as_array()
...
/// `remote_open`: `fapi_open_orders` yanıtı. `local`: `exchange=binance`, `segment=futures`, `venue_order_id` dolu.
pub fn reconcile_binance_futures_open_orders(
    remote_open: &Value,
    local: &[ExchangeOrderVenueSnapshot],
) -> Result<ReconcileReport, ExecutionError> {
    let remote_ids = parse_binance_open_order_ids(remote_open)?;
    ...
    let mismatches = local_submitted_not_open + remote_unknown;
    let notes = if mismatches == 0 {
        "Futures açık emir listesi ile yerel venue_order_id eşlemesi tutarlı (submitted ve bilinmeyen uzaktan emir yok)."
            .into()
    } else {
        format!(
            "Dikkat: {local_submitted_not_open} yerel submitted borsada açık değil; {remote_unknown} borsa açık emri yerelde venue_order_id ile bulunamadı."
