...
**Tekil sürüm numarası:** Aynı `NNNN_` öneki iki dosyada kullanılamaz (`_sqlx_migrations` satır başına tek checksum). Şu an çoğu şema **`migrations/0001_qtss_baseline.sql`** içinde birleştirilmiştir; yeni şema değişikliği için genelde **`0002_*.sql`** eklenir (veya baseline yeniden üretilir — `migrations/README.md`). `qtss-sync-sqlx-checksums` çift öneği reddeder.
