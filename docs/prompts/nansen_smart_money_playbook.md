...
2. **Rule-based intake (no LLM credits)** — `qtss-worker` `intake_playbook_engine` (off by default): prefer **`system_config`** module `worker` — `intake_playbook_loop_enabled` → `{ "enabled": true }`, `intake_playbook_tick_secs` → `{ "secs": 300 }`. Env `QTSS_INTAKE_*` is fallback; `QTSS_CONFIG_ENV_OVERRIDES=1` makes env win. Writes `intake_playbook_runs` + `intake_playbook_candidates` from `data_snapshots` (`nansen_token_screener`, `nansen_netflows`, `nansen_flow_intelligence`, `nansen_perp_trades`, Binance `binance_premium_btcusdt` / `ethusdt`). Optional **`notify_outbox`** alerts: `intake_playbook_notify_enabled`, `intake_playbook_notify_channels` (seed migration `0007`; comma-separated value) — when **market mode** changes vs the previous run and when **ten_x_alert** triggers (requires `notify_outbox` worker loop). **ten_x** dedupe: `intake_playbook_notify_ten_x_dedupe_secs` / env `QTSS_INTAKE_PLAYBOOK_NOTIFY_TEN_X_DEDUPE_SECS` (default 86400; `0` disables). **API:** `GET /api/v1/analysis/intake-playbook/latest?playbook_id=market_mode` (and other playbook ids), `GET /api/v1/analysis/intake-playbook/recent?limit=50`. **Promote (trader/admin):** `POST …/intake-playbook/promote`, `POST …/intake-playbook/promote-bulk` (max 25) — creates `engine_symbols` **disabled** by default; existing series is linked without changing `enabled`. Promoted rows use labels such as `intake:…`; **`engine_analysis_loop`** treats any `label` whose prefix matches `engine_light_pipeline_label_prefix` (default `intake:`, env `QTSS_ENGINE_LIGHT_PIPELINE_LABEL_PREFIX`, migration `0005_engine_light_pipeline_label_prefix`) as a **light pipeline**: no classic `formations` scan (stub snapshot), TBM structure pillar without pattern boost; `trading_range`, `signal_dashboard`, and confluence still run. Heuristics are best-effort; use LLM prompts below for refinement.
