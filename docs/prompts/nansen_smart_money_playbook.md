...
2. **Rule-based intake (no LLM credits)** — `qtss-worker` `intake_playbook_engine` (off by default): set `QTSS_INTAKE_PLAYBOOK_ENABLED=1` or `system_config` `worker` / `intake_playbook_loop_enabled` → `{ "enabled": true }`. Tick: `QTSS_INTAKE_PLAYBOOK_TICK_SECS` or `intake_playbook_tick_secs` (default 300s). Writes `intake_playbook_runs` + `intake_playbook_candidates` from `data_snapshots` (`nansen_token_screener`, `nansen_netflows`, `nansen_flow_intelligence`, `nansen_perp_trades`, Binance `binance_premium_btcusdt` / `ethusdt`). Optional **`notify_outbox`** alerts: `QTSS_INTAKE_PLAYBOOK_NOTIFY_ENABLED=1`, `QTSS_INTAKE_PLAYBOOK_NOTIFY_CHANNELS=telegram` — when **market mode** changes vs the previous run and when **ten_x_alert** triggers (requires `notify_outbox` worker loop). **API:** `GET /api/v1/analysis/intake-playbook/latest?playbook_id=market_mode` (and other playbook ids), `GET /api/v1/analysis/intake-playbook/recent?limit=50`. **Promote (trader/admin):** `POST …/intake-playbook/promote`, `POST …/intake-playbook/promote-bulk` (max 25) — creates `engine_symbols` **disabled** by default; existing series is linked without changing `enabled`. Promoted rows use labels such as `intake:…`; **`engine_analysis_loop`** treats any `label` whose prefix matches `engine_light_pipeline_label_prefix` (default `intake:`, env `QTSS_ENGINE_LIGHT_PIPELINE_LABEL_PREFIX`, migration `0005_engine_light_pipeline_label_prefix`) as a **light pipeline**: no classic `formations` scan (stub snapshot), TBM structure pillar without pattern boost; `trading_range`, `signal_dashboard`, and confluence still run. Heuristics are best-effort; use LLM prompts below for refinement.
