...
3. Emir + mutabakat (`reconcile`) sıklığı ve worker entegrasyonu — **kısmen:** spot `binance_spot_reconcile.rs` (`QTSS_RECONCILE_BINANCE_SPOT_*`, `QTSS_RECONCILE_BINANCE_SPOT_PATCH_STATUS`) + futures `binance_futures_reconcile.rs` (`QTSS_RECONCILE_BINANCE_FUTURES_*`, `QTSS_RECONCILE_BINANCE_FUTURES_PATCH_STATUS`); API `POST /api/v1/reconcile/binance` ve `POST /api/v1/reconcile/binance/futures`. Uyumsuz `submitted` satırları `reconciled_not_open` yapılır (`ExchangeOrderRepository::mark_submitted_reconciled_not_open_by_venue_ids`); yanıtta `status_updates_applied` (API’de); venue bazında FILLED/CANCELED ayrımı için ek order query sonraki iterasyon.  
...
**Önerilen bir sonraki teknik adım (küçük kapsam):** §9.1 satır 1–8’deki ürün önceliğine göre kısmi madde derinleştirmesi veya worker/API için Prometheus metrik genişletmesi. Büyük özellik: reconcile sonrası borsa `order` sorgusu ile kesin durum (FILLED/CANCELED) veya copy trade WebSocket.
