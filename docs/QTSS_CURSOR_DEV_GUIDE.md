...
1. Kimlik / RBAC ince taneli izinler ve denetim derinliği — **kısmen:** JWT `permissions` (`qtss:read` \| `qtss:ops` \| `qtss:admin`), yeni tokenlarda claim; eski tokenlarda `require_jwt` sonrası `roles` ile doldurulur; `GET /api/v1/me` etkin `permissions` döner (JWT + `user_permissions` birleşimi, `QTSS_JWT_MERGE_DB_PERMISSIONS` ile kapatılabilir). Admin: `GET`/`PUT /api/v1/users/{user_id}/permissions` (aynı `org_id`). Migration `0013_user_permissions.sql`. Daha ince stringler, kurumlar arası yönetim ve denetim derinliği sonraki iterasyon.  
