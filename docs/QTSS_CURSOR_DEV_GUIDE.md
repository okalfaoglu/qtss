...
1. Kimlik / RBAC ince taneli izinler ve denetim derinliği — **kısmen:** JWT `permissions` (`qtss:read` \| `qtss:ops` \| `qtss:admin` \| `qtss:audit:read`); `admin` rolüne `qtss:audit:read` dahil; `GET /api/v1/audit/recent` → `qtss:admin` **veya** `qtss:audit:read` (`require_audit_read`); sorgu `?kind=` → `details->>'kind'` (`user_permissions_replace`, `http_mutation`, …). `user_permissions` ile viewer’a yalnız denetim okuma verilebilir. Admin: `GET`/`PUT /api/v1/users/{user_id}/permissions` (yalnız `qtss:admin`). `QTSS_AUDIT_HTTP=1` iken `audit_log.details`: `PUT …/permissions` → `schema_version` + `user_permissions_replace`; diğer mutasyonlar → `http_mutation`. Migration `0040_user_permissions.sql`, `0041_audit_log_details.sql`. Kurumlar arası yönetim ve daha zengin olay türleri sonraki iterasyon.  
...
3. Emir + mutabakat (`reconcile`) sıklığı ve worker entegrasyonu — **kısmen:** spot `binance_spot_reconcile.rs` (`QTSS_RECONCILE_BINANCE_SPOT_*`, `PATCH_STATUS`, isteğe bağlı `REFINE_ORDER_STATUS` + `REFINE_MAX`) + futures eşdeğeri; ortak patch `crates/qtss-reconcile` (`apply_binance_*_open_orders_patch`). API `POST /api/v1/reconcile/binance` ve `POST /api/v1/reconcile/binance/futures` (spot’ta da DB patch). İsteğe bağlı `GET .../order` ile `filled` / `partially_filled` / `canceled` (`exchange_order_status_from_binance_json`); kalan `submitted` → `reconciled_not_open`. Yanıtta `status_updates_applied`. İnce ayar: gerçekleşen miktar / ücret özeti için ek alanlar veya WebSocket fill sonraki iterasyon.  
...
9. HA / observability — **kısmen:** API: `GET /live`, `GET /ready`, `/metrics` (`qtss_build_info`, `qtss_http_requests_total`, `QTSS_METRICS_TOKEN`). Worker: `QTSS_WORKER_HTTP_BIND` ile aynı yollar (`worker_probe_http.rs`). Dağıtılmış HA, trace örnekleme, alarm kuralları sonraki iterasyon.  
10. Depo CI — **kısmen:** `.github/workflows/rust-ci.yml` — push/PR: `cargo check --workspace --all-targets`, `cargo test --workspace` (birim / entegrasyon testleri Postgres istemez; DB’li test eklenirse `services:` postgres sonraki iterasyon), `cargo audit` (`cargo-audit@0.22.1`), `web/` `npm ci` + `npm run build` (Node 20). Sonraki iterasyon: Postgres servisli entegrasyon job’u veya OS matrisi.  

**Önerilen bir sonraki teknik adım (küçük kapsam):** §9.1 satır 1–8’deki ürün önceliğine göre kısmi madde derinleştirmesi veya worker/API için Prometheus metrik genişletmesi. Büyük özellik: copy trade Binance user stream veya reconcile metrikleri.
