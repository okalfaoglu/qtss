...
1. Kimlik / RBAC ince taneli izinler ve denetim derinliği — **kısmen:** JWT `permissions` (`qtss:read` \| `qtss:ops` \| `qtss:admin` \| `qtss:audit:read`); `admin` rolüne `qtss:audit:read` dahil; `GET /api/v1/audit/recent` → `qtss:admin` **veya** `qtss:audit:read` (`require_audit_read`). `user_permissions` ile viewer’a yalnız denetim okuma verilebilir. Admin: `GET`/`PUT /api/v1/users/{user_id}/permissions` (yalnız `qtss:admin`). `PUT` izinleri `QTSS_AUDIT_HTTP=1` iken `audit_log.details`. Migration `0040_user_permissions.sql`, `0041_audit_log_details.sql`. Kurumlar arası yönetim ve tam olay şeması sonraki iterasyon.  
